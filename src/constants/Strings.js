const strings = {
    loginHeading1: 'Welcome Back',
    loginHeading2: 'Discover Limitless Choices and Unmatched',
    loginHeading3: 'Convenience',
    signUpHeading: 'Lets Create Your Account',
    signUpHeading2: 'Explore Different Cultures and Communities',




    //EmailVerification Screen
    EmailVerify: 'Verify Your Email Address',

    //Forget Password
    ForgetPassword: 'Forgot Your Password?',
    ForgetPasswordHeading: 'Do not worry sometimes people can forget too, enter your email and we will send you forget password reset link'
}

export default strings